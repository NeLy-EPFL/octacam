...
void Camera::grab(int n_frames) {
    ...
    auto writer = cv::VideoWriter(filename, cv::VideoWriter::fourcc('a', 'v', 'c', '1'), 30, cv::Size(width, height), false);
    ...
    for (int i = 0; i < n_frames; ++i) {
        ...
        if (ptrGrabResult->GrabSucceeded()) {
            ...
            writer.write(cv::Mat(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8UC1, (void*) pImageBuffer));
        } else {
...
void CameraSystem::record(int n_frames) {
    ...
    for (auto& camera : cameras) {
        grab_threads.emplace_back([&camera, n_frames]() {
            camera.grab(n_frames);
        });
    }
